...
Le choix suit catégorie → espèce → confirmation. Non revient aux espèces ; B remonte d'un niveau et, au niveau catégorie, annule sans attribution. Pour Évoli, une seconde liste lit uniquement les évolutions par objet déjà définies dans les données ; elle enregistre une préférence ou « Plus tard ». Elle ne donne pas de pierre et ne provoque pas d'évolution. `OnStarterConfirmed` est le point de branchement prévu pour les futurs objets associés : aucune table d'objets n'est définie.
