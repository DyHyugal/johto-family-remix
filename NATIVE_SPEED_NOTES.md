...
Validation commands: `make hns -j4`; `make check BUILD=hns TESTS=Native -j4`; existing Audio and Settings tests. The Ubuntu-24.04 WSL toolchain is now available: the production build and Native (3), Audio (6), and Settings (3) tests passed on 2026-09-22. Native includes a complete battle turn parametrized at all four speeds. The dedicated validation workflow also includes the Family starter/egg tests. See the delivered validation report for the final combined-build results and manual checks still outstanding. A workflow definition or an unrun test is not a successful validation.
